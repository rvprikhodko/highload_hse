...
Нетрудно заметить, что количество RPS и трафика - огромные. Тогда пускай скажем, что пропускная способность каждого сервера - 80 Гбит/с. С учетом потери в 15%:

$$
bandwith = 0.2 * (1-0.85) = 68
$$

$$
TpS = \frac{292 Тбит/с}{x} = 68 Гбит/c,
$$
...
$$
x_{NA} = \frac{87.6}{68}=1320
$$

Таким образом, для поддержания пропускной способности серверов в Северной Америке необходимо 1320 серверов.
...
$$
x_{SA} = \frac{29.2}{68}=440
$$

В Южной Америке необходимо иметь 440 серверов. Большинство должны быть расположены на северно-западном и центрально-восточном побережьях.
...
$$
x_{EU} = \frac{78.84}{68}=1187
$$

Для Европы нужно 1187 сервера. 
...
x_{AS}=\frac{96.36}{68}=1451

Для Азии нужно 1451 серверов. Большинство из них должны быть в Индии. Китайское население не приняло Youtube и пользуются импортозамещением.

### Итог

| Регион           | Количество серверов |
|------------------|---------------------|
| Северная Америка | 1320                |
| Европа           | 1187                 |
| Азия             | 1451                 |
| Южная Америка    | 440                |
...
| Нагрузка                             | 292 Тбит/c |
| Пропускная способность сервера       | 80 Гбит/с  |
| RPS Nginx [7]                        | 600 000    |
...
Найдем нагрузку на один сервер:

$$
Traffic = \frac{292}{x} = 80 Гбит/с
$$

$$
Пропускная способность сервера = 0.85 * 80 = 68 Гбит/с
$$

$$
Servers = \frac{292}{68} = 4399
$$
...
$$
Servers_{Пропускная способность сервера} = \frac{68}{68} = 1
$$

По RPS:

$$
Servers_{RPS} = \frac{\frac{4 170 000}{3734}}{600000} = 0.002
$$

Тогда получаем 4399 серверов. По учету отказоустойчивости - 6599.

Итоговая сводная таблица:

| Характеристика                                                         |  Значение |
|:-----------------------------------------------------------------------|----------:|
| Ожидаемая пропускная способность сервера                               | 68 Гбит/с |
| Количество серверов необходимое для обеспечения пропускной способности |      4399 |
| Количество серверов, необходмое для обеспечения RPC                    |         7 |
| Итоговое количество серверов                                           |      4399 |
| Итоговое количество серверов с учётом резервирования N * 1.5           |      6599 |

Данные вычисления подтверждают, что выбранного количества серверов хватает для безотказной обработки трафика и RPS.

Если использовать распределение серверов по ДЦ как у Google, то один ДЦ будет содержать 400-600 серверов.
...
| ------------- | ------------- | ------------ |--------|
| Users         | 0.6 КБ        | 2 млрд       | 1.2 ТБ |
| Channels      | 1.2 КБ        | 200 млн      | 240 ГБ |
| Videos        | 1.35 КБ       | 500 млн      | 675 ГБ |
| VideoFiles    | 324 байта     | 2 млрд       | 648 ГБ |
| VideoChunks   | 280 байт      | 100 млрд     | 28 ТБ  |
| VideoStats    | 40 байт       | 500 млн      | 20 ГБ  |
| Likes         | 24 байта      | 20 млрд      | 480 ГБ |
| Comments      | 561 байт      | 10 млрд      | 5.6 ТБ |
| Subscriptions | 24 байта      | 5 млрд       | 120 ГБ |
| WatchHistory  | 32 байта      | 100 млрд     | 3.2 ТБ |
| WatchSessions | 104 байта     | 20 млрд      | 2.1 ТБ |
...
| --------------- | -------------- |--------------|-------------------| -------------------- |-----------------------------------|
| Users           |  2 млрд        | 1.2 ТБ       | Средняя           | Низкая               | Аутентификация и профиль          |
| Channels        |  200 млн       | 240 ГБ       | Средняя           | Низкая               | Относительно стабильная таблица   |
| Videos          | 500 млн        | 675 ГБ       | Высокая           | Средняя              | Основная точка входа для контента |
| VideoFiles      | 2 млрд         | 648 ГБ       | Высокая           | Средняя              | Используется при выборе качества  |
| VideoChunks     | 100 млрд       | 28 ТБ        | Очень высокая     | Низкая               | Основная нагрузка стриминга       |
...
| Comments        |  10 млрд       | 5.6 ТБ       | Высокая           | Высокая              | Перекос на популярные видео       |
| Subscriptions   | 5 млрд         | 120 ГБ       | Высокая           | Средняя              | Используется для ленты            |
| WatchHistory    | 100 млрд       | 3.2 ТБ       | Средняя           | Очень высокая        | Поток событий просмотров          |
| WatchSessions   | 20 млрд        | 2.1 ТБ       | Низкая            | Высокая              | Аналитика                         |
...
| VideoStats                 | ClickHouse         | По `video_id` + shard_index                           | Без партиций                     | KEY(`video:{video_id}`)                                                                                                                              | Репликация в Redis-кластер, snapshot + AOF              |
| Likes / Dislikes           | Vitess             | По составному ключу (`video_id`, month(`created_at`)) | По месяцу `created_at`           | PK(`user_id`, `video_id`), INDEX(`video_id`, `created_at`), INDEX(`user_id`)                                                                         | 2–3 реплики                                             |
...
| ------------------- |------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| PostgreSQL          | Используется потоковая репликация (Primary + 2–3 Replica). Полные резервные копии выполняются средствами pgBackRest с реплик БД один раз в неделю. Для восстановления до произвольного момента времени используется WAL                                                                                                                                                                                                                                                                                                                                                                                                                |
...
## Алгоритм рекомендательной системы

Рекомендательная система формирует персонализированный список видео для каждого пользователя заранее.
...
1. События пользователей (просмотры, лайки, поиск, подписки) отправляются в Kafka
2. События сохраняются в ClickHouse для аналитики
3. Рекомендательный сервис периодически обрабатывает накопленные данные
4. Для каждого пользователя формируется профиль интересов:
   * категории видео
   * предпочитаемые авторы
   * длительность видео
   * интересующие темы
5. На основе профиля подбираются кандидаты:
   * видео из подписок
   * похожие видео
   * популярные видео
   * новые видео
   * shorts
6. Для каждого кандидата вычисляется скор с учетом:
   * вероятности просмотра
   * вероятности досмотра
   * CTR
   * retention
   * свежести видео
   * популярности
7. Видео сортируются по скору
8. Готовый feed сохраняется в Redis
9. При открытии главной страницы backend читает уже готовый feed из Redis
10. При нехватке рекомендаций выполняется динамическая догрузка новых кандидатов

---

## Алгоритм поиска кандидатов

Поиск кандидатов используется для быстрого поиска подходящих видео.

1. Для каждого видео строится embedding-вектор
2. Для каждого пользователя строится embedding интересов
3. Векторный поиск выполняется через ANN-индекс
4. Находятся наиболее похожие видео
5. Из результатов удаляются:
   * уже просмотренные видео
   * скрытые видео
   * неподходящий контент
6. Полученный список передается в ranking service

## Алгоритм скоринга

Скор определяет порядок видео в рекомендациях.

1. Для каждого кандидата собираются признаки:
   * CTR
   * среднее удержание
   * популярность
   * тематика
   ...
   * регион
   * свежесть
2. ML-модель вычисляет вероятность:
   * клика
   * полного просмотра
   * взаимодействия
3. Итоговый score вычисляется как взвешенная сумма факторов
4. Видео сортируются по score
5. Top-N рекомендаций сохраняются в feed пользователя

-----------------
## Алгоритм авторизации запросов

Авторизация выполняется на уровне API Gateway.

1. Клиент отправляет запрос с JWT access token 
2. API Gateway извлекает токен
3. Gateway проверяет:
   * цифровую подпись JWT
   * срок действия
4. Из JWT извлекаются:
   * user_id
5. Если проверка успешна:
   * запрос передается во внутренний сервис
   * user context прокидывается через internal headers
6. Backend сервис выполняет бизнес-проверки:
   * права на редактирование видео
   * права на комментарии
   * ограничения доступа
7. При ошибке авторизации:
   * возвращается HTTP 401 / 403
...
![project_scheme](new_project_scheme.png)

## Описание схемы проекта

Пользовательские запросы поступают через L4 Load Balancer и CDN, после чего попадают в L7 Origin Group и API Gateway, 
который маршрутизирует их в соответствующие микросервисы. За воспроизведение видео отвечает Playback Service, 
который получает метаданные роликов и ссылки на HLS/DASH-манифесты, 
а сами видеосегменты раздаются через CDN из объектного хранилища. 
Загрузка новых видео выполняется через Upload & Processing Service: исходные файлы сохраняются в Object Storage, 
после чего через Kafka создаются задачи на транскодирование, которые обрабатываются FFmpeg Workers.

Video Core Service управляет метаданными видео, каналами и статистикой просмотров, используя PostgreSQL в качестве основной OLTP-базы данных. 
Social Interaction Service отвечает за комментарии, лайки, подписки и другие социальные взаимодействия пользователей. 
User Service хранит информацию о пользователях и профилях, 
а Auth Service реализует регистрацию, аутентификацию, выдачу JWT-токенов и управление пользовательскими сессиями, 
используя PostgreSQL и Redis для хранения данных и кэширования.

Feed & Recommendation Service формирует персонализированную ленту рекомендаций на основе истории просмотров, подписок и пользовательской активности. 
Для аналитики и обработки событий используется Kafka и ClickHouse, 
где накапливаются данные о просмотрах, поисковых запросах и взаимодействиях пользователей. 
Поиск видео реализован отдельным Search Service, который обращается к Elasticsearch через Search Load Balancer и выполняет полнотекстовый поиск по названиям, описаниям, тегам и каналам. 
Такая архитектура позволяет независимо масштабировать отдельные компоненты системы и обеспечивать стабильную работу видеохостинга при высокой нагрузке.
...
| ---------------- | ----------- | --------------------------------- | ------------------- | ------------------------- | -------------------------- |
| Хранилище        | Физический  | 64 CPU / 256 GB RAM / 100 TB disk | 2200                | 8000                      | 17 600 000                 |
| Транскодирование | Физический  | 32 CPU / 128 GB RAM / GPU         | 5500                | 3000                      | 16 500 000                 |
| PostgreSQL       | Физический  | 32 CPU / 128 GB RAM / 4 TB NVMe   | 120                 | 4000                      | 480 000                    |
| Redis            | Физический  | 16 CPU / 256 GB RAM / 2 TB NVMe   | 50                  | 5000                      | 250 000                    |
| ClickHouse       | Физический  | 32 CPU / 128 GB RAM / 20 TB disk  | 35                  | 6000                      | 210 000                    |
| Kafka            | Физический  | 16 CPU / 64 GB RAM / 10 TB disk   | 30                  | 4000                      | 120 000                    |
| Elasticsearch    | Физический  | 16 CPU / 64 GB RAM / 8 TB disk    | 30                  | 4000                      | 120 000                    |
| Backend          | ВМ          | 32 CPU / 16 GB RAM                | 700                 | 1000                      | 700 000                    |
| L7 Balancer      | ВМ          | 128 CPU / 64 GB RAM               | 600                 | 2000                      | 1 200 000                  |
| Kubernetes       | ВМ          | 8 CPU / 16 GB RAM                 | 70                  | 500                       | 35 000                     |
